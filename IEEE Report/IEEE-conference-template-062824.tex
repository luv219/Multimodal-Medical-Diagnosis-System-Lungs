...
architecture-specific reshape transforms. Results and per-class
AUC-ROC comparisons are reported across all 14 disease categories
...
\midrule
Atelectasis         & \textit{[TBD]} & \textit{[TBD]} \\
Cardiomegaly        & \textit{[TBD]} & \textit{[TBD]} \\
Effusion            & \textit{[TBD]} & \textit{[TBD]} \\
Infiltration        & \textit{[TBD]} & \textit{[TBD]} \\
Mass                & \textit{[TBD]} & \textit{[TBD]} \\
Nodule              & \textit{[TBD]} & \textit{[TBD]} \\
Pneumonia           & \textit{[TBD]} & \textit{[TBD]} \\
Pneumothorax        & \textit{[TBD]} & \textit{[TBD]} \\
Consolidation       & \textit{[TBD]} & \textit{[TBD]} \\
Edema               & \textit{[TBD]} & \textit{[TBD]} \\
Emphysema           & \textit{[TBD]} & \textit{[TBD]} \\
Fibrosis            & \textit{[TBD]} & \textit{[TBD]} \\
Pleural\_Thickening & \textit{[TBD]} & \textit{[TBD]} \\
Hernia              & \textit{[TBD]} & \textit{[TBD]} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{CNN-TE} \\
\midrule
Atelectasis         & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Cardiomegaly        & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Effusion            & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Infiltration        & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Mass                & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Nodule              & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Pneumonia           & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Pneumothorax        & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Consolidation       & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Edema               & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Emphysema           & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Fibrosis            & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Pleural\_Thickening & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
Hernia              & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
\midrule
\textbf{Macro AUC}  & \textit{[TBD]} & \textit{[TBD]} & \textit{[TBD]} \\
\bottomrule
\end{tabular}
\end{table}
...
AUC, weighted F1 score, parameter count, and inference throughput
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{mAUC} & \textbf{wF1} &
\textbf{Params} & \textbf{Img/s} \\
\midrule
DenseNet-121  & \textit{[TBD]} & \textit{[TBD]} & $\sim$7M  & \textit{[TBD]} \\
Swin-Tiny     & \textit{[TBD]} & \textit{[TBD]} & $\sim$30M & \textit{[TBD]} \\
CNN-TE (ours) & \textit{[TBD]} & \textit{[TBD]} & $\sim$27M & \textit{[TBD]} \\
\bottomrule
\end{tabular}
\end{table}
...
DenseNet-121's dense connectivity pattern~\cite{huang2017} creates
feature reuse pathways that are well-suited to detecting
locally-textured pathologies such as Nodule and Mass, where fine-grained
texture and boundary information are diagnostically informative.
The inductive spatial bias of convolutional layers produces
translationally equivariant features that generalise well from limited
training examples of rare diseases. The Swin-Tiny's shifted windowed
self-attention~\cite{liu2021} captures global spatial context across
the full radiograph field, which is expected to benefit pathologies
with diffuse or bilateral presentation patterns such as Cardiomegaly
and Effusion, where the relationship between anatomical structures
across the image is more informative than local texture alone.
...
suggesting that their combination in CNN-TE may capture complementary
discriminative signal across the 14-class spectrum.
...
evidence each model uses. Full quantitative results will be reported
